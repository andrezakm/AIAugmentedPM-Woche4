...
|----|-----------|------------|----------------|----------|
| E1 | App öffnet fehlerfrei | `prototype/case1/app.html` per Doppelklick im Browser öffnen, Konsole (DevTools) prüfen | Keine Konsolenfehler, Inhalt sichtbar | FAIL |
...
| E8 | Zähler aktualisiert sich | Filter auf eine einzelne Quelle setzen, z.B. nur "Slack" | Sidebar zeigt die Anzahl der tatsächlich sichtbaren Cluster, bei nur "Slack" also "3 von 4 Clustern" | FAIL |
| E9 | Kein-Treffer-Hinweis bei leerem Ergebnis | Alle Quellen abwählen | Hinweistext erscheint, kein leerer Hauptbereich | FAIL |
| E10 | Keine Hardcoded-Daten | `data.js` und `app.html` durchsehen | Cluster-Inhalte kommen aus `data.js` (`window.DATA`), nicht hardcoded in `app.html` | FAIL |
| E11 | Sonstiges bleibt außen vor | App durchsehen, alle Quellen aktiv | Die Einzelfälle aus dem Abschnitt „Sonstiges / Einzelfälle" erscheinen nicht als Cluster-Karte | FAIL |
